...
// 해결: 컨트롤러마다 if(session.getAttribute("userId") == null)을 쓰는 건 비효율적입니다. Interceptor를 설정해서 로그인 필요한 페이지 접근 시 자동으로 로그인 창으로 튕겨내야 합니다.
// 2. 아이디 저장 (Cookie)
// 상황: 사용자가 매번 아이디를 치기 귀찮아합니다.
// 해결: 로그인 화면의 [v] 아이디 저장 체크박스 기능입니다. 로그인 성공 시 **쿠키(Cookie)**에 아이디를 암호화 없이 저장해두고, 다음 접속 시 
// Controller
...
// 예: UPDATE USERS SET LAST_LOGIN_DATE = SYSDATE, LOGIN_IP = #{ip} WHERE USER_ID = #{id}
// 회원 테이블 관리 차원에서 중요합니다.
// 추천: 가장 시급한 것은 1번 인터셉터입니다. 이게 없으면 기껏 만든 로그인이 무용지물이 될 수 있습니다. 혹시 인터셉터 설정도 진행하실 예정인가요?
